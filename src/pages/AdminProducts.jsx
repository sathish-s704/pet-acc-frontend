...
function AdminProducts() {
  ...
  const getAuthConfig = (isMultipart = false) => {
    // Read token from stored user object for consistency with AuthContext
  ...
  const res = await axiosInstance.get('/products', getAuthConfig());
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this product?')) {
      try {
  await axiosInstance.delete(`/products/${id}`, getAuthConfig());
        fetchProducts();
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingProductId) {
  const response = await axiosInstance.put(`/products/${editingProductId}`, data, getAuthConfig(true));
        console.log('Update response:', response.data);
      } else {
  const response = await axiosInstance.post('/products', data, getAuthConfig(true));
        ...
        <tbody>
          {products.map(product => (
            <tr key={product._id}>
              <td>
                <img 
                  src={product.imageUrl ? getAssetUrl(product.imageUrl) : '/pet images/collar.jpeg'} 
                  alt={product.name}
                  width="50" 
                  height="50" 
